...
def _apply_simplipy_patch():
    try:
        from aiohttp import ClientResponseError
        import simplipy.api

        if getattr(simplipy.api.API, '_patched_for_403', False):
            return

        _original_post_init = simplipy.api.API._async_post_init

        async def _patched_post_init(self):
            try:
                await _original_post_init(self)
            except ClientResponseError as err:
                if err.status == 403 and self.access_token:
                    try:
                        payload_b64 = self.access_token.split('.')[1]
                        payload_b64 += '=' * (-len(payload_b64) % 4)
                        payload = json.loads(base64.b64decode(payload_b64))
                        uid = payload.get('http://simplisafe.com/uid')
                        if uid:
        ...
        simplipy.api.API._async_post_init = _patched_post_init
        simplipy.api.API._patched_for_403 = True
    except Exception:
        pass

_apply_simplipy_patch()
# END_PATCH
